#!/usr/bin/env python3
"""
Telemetry collector for the Collective-Aware In-Switch Telemetry prototype.

Listens on the mirror-destination host's interface for the compact
straggler reports cloned by collective_telemetry.p4 (EtherType 0x1234)
and decodes/prints them.

This never sees "normal" collective traffic -- only clones the switch
generates when the intra-operation skew exceeds the operator-defined
threshold, i.e. event-driven, straggler-triggered reporting.

Usage:
    python3 collector.py [iface]      # default iface: h5-eth0
"""
...
REPORT_LEN = 30  # contextId(2)+opType(1)+stragglerId(1)+tFast(6)+tSlow(6)+skew(6)+qdepth(4)+qdelay(4)
...
def main():
    ...
    print('[collector] listening on %s for telemetry reports (EtherType 0x%04x)...'
          % (iface, REPORT_ETHERTYPE))
